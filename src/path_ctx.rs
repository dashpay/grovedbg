...
use crate::bytes_utils::{bytes_by_display_variant, BytesDisplayVariant};
...
impl PathSegment {
    ...
    pub fn display(&self) -> BytesDisplayVariant {
        self.display
    }
}
...
impl<'c> Path<'c> {
    ...
    pub fn for_display_mut<T>(&self, f: impl FnOnce(&mut BytesDisplayVariant) -> T) -> Option<T> {
        if let Some(id) = self.head_slab_id {
            let mut slab = self.ctx.slab.borrow_mut();
